...
class RssFeedsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create rss_feed" do
    assert_difference('RssFeed.count') do
      post rss_feeds_url, params: { rss_feed: { link: @rss_feed.link, name: @rss_feed.name } }
    end
  ...
  test "should update rss_feed" do
    patch rss_feed_url(@rss_feed), params: { rss_feed: { link: @rss_feed.link, name: @rss_feed.name } }
